...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname IN ('orders_seller_company_id_fkey','fk_orders_seller_company')) THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname IN ('orders_buyer_company_id_fkey','fk_orders_buyer_company')) THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname IN ('orders_factory_company_id_fkey','fk_orders_factory_company')) THEN
...
WITH pick AS (
  -- 修正(Claude 0723 审): 按已定案模型 客户→巴匕→真实工厂，trader 边优先，
  -- 不用 edge_count=1(0625 给18买家补了 →BABI trader 通用边，多数买家有2~4条边，
  -- 原规则会把 73/131 单误判歧义，等于本功能失效)。
  SELECT DISTINCT ON (pr.company_code_a)
         pr.company_code_a,
         pr.company_code_b AS supplier_code,
         COALESCE(NULLIF(btrim(pr.role_at_hop), ''),
                  CASE WHEN upper(pr.company_code_b) = 'BABI' THEN 'trader' ELSE 'factory' END) AS role_at_hop
  FROM partner_relationships pr
  WHERE pr.relationship_type = 'customer_factory'
    AND COALESCE(pr.status, 'active') = 'active'
  ORDER BY pr.company_code_a,
           (COALESCE(NULLIF(btrim(pr.role_at_hop),''),'') = 'trader') DESC,
           pr.company_code_b
),
unique_edges AS (
  SELECT p.company_code_a, p.supplier_code, p.role_at_hop, c.id AS supplier_company_id
  FROM pick p
  JOIN companies c ON c.code = p.supplier_code
)
UPDATE orders o
...
-- ---------------------------------------------------------------------
-- M3 第7步（Damon 0723 定调「整表带过来，靠展示/不展示控制」）
-- 不手挑字段：从 information_schema 生成式全列带过来，以后主表加列自动跟上。
-- 露不露交给 field_visibility_config，不在 lookup 层裁剪。
-- ---------------------------------------------------------------------

-- 9 个公司角色 × companies 全部列
WITH roles(role_key, target_field_key, relation_key) AS (
  VALUES
    ('seller',   'seller_company_id',   'orders.seller_company_id__companies.id'),
    ('buyer',    'buyer_company_id',    'orders.buyer_company_id__companies.id'),
    ('customer', 'customer_company_id', 'orders.customer_company_id__companies.id'),
    ('issuing',  'issuing_company_id',  'orders.issuing_company_id__companies.id'),
    ('operating','operating_company_id','orders.operating_company_id__companies.id'),
    ('owner',    'owner_company_id',    'orders.owner_company_id__companies.id'),
    ('factory',  'factory_company_id',  'orders.factory_company_id__companies.id'),
    ('supplier', 'supplier_company_id', 'orders.supplier_company_id__companies.id'),
    ('trader',   'trader_company_id',   'orders.trader_company_id__companies.id')
),
cols AS (
...
gen AS (
  SELECT 'orders.'||r.role_key||'.'||c.column_name AS lookup_key,
         'orders' AS module_key, r.target_field_key, r.relation_key,
         'companies' AS related_module, c.column_name AS related_field_key,
         'snapshot' AS mode, true AS is_readonly
  FROM roles r CROSS JOIN cols c
)
INSERT INTO field_lookups (lookup_key, module_key, target_field_key, relation_key,
                           related_module, related_field_key, mode, is_readonly, status)
SELECT lookup_key, module_key, target_field_key, relation_key,
       related_module, related_field_key, mode, is_readonly, 'active'
FROM gen
WHERE NOT EXISTS (SELECT 1 FROM field_lookups fl WHERE fl.lookup_key = gen.lookup_key);

-- order_line_items.product_id → products 全部列
WITH cols AS (
  SELECT column_name FROM information_schema.columns
  WHERE table_schema='public' AND table_name='products' AND column_name <> 'id'
),
gen AS (
  SELECT 'order_line_items.product.'||c.column_name AS lookup_key,
         'order_line_items' AS module_key, 'product_id' AS target_field_key,
         'order_line_items.product_id__products.id' AS relation_key,
         'products' AS related_module, c.column_name AS related_field_key,
         'snapshot' AS mode, true AS is_readonly
  FROM cols c
)
INSERT INTO field_lookups (lookup_key, module_key, target_field_key, relation_key,
...
-- 展示层：每条 lookup 生成一行可见性配置。默认只露核心字段，其余带过来但不展示。
WITH defaults(col, vis, ord) AS (
  VALUES ('name_cn',true,10),('name_en',true,20),('code',true,30),('tax_id',true,40),
         ('address',true,50),('bank_accounts',true,60),('short_name',true,70),
         ('contact_name',true,80),('contact_phone',true,90),('country',true,100)
)
INSERT INTO field_visibility_config (screen, field_key, role, visible, editable, sort_order)
SELECT 'orders', fl.lookup_key, 'admin',
       COALESCE(d.vis,false), false, COALESCE(d.ord, 900)
FROM field_lookups fl
LEFT JOIN defaults d ON d.col = fl.related_field_key
WHERE fl.module_key='orders' AND fl.related_module='companies'
  AND NOT EXISTS (SELECT 1 FROM field_visibility_config v
                  WHERE v.screen='orders' AND v.field_key=fl.lookup_key AND v.role='admin');

WITH defaults(col, vis, ord) AS (
  VALUES ('sku',true,10),('name_cn',true,20),('name_en',true,30),('brand',true,40),
         ('spec',true,50),('barcode',true,60),('hs_code',true,70),('factory_code',true,80),
         ('net_weight',true,90),('gross_weight',true,100),('cbm',true,110)
)
INSERT INTO field_visibility_config (screen, field_key, role, visible, editable, sort_order)
SELECT 'order_line_items', fl.lookup_key, 'admin',
       COALESCE(d.vis,false), false, COALESCE(d.ord, 900)
FROM field_lookups fl
LEFT JOIN defaults d ON d.col = fl.related_field_key
WHERE fl.module_key='order_line_items' AND fl.related_module='products'
  AND NOT EXISTS (SELECT 1 FROM field_visibility_config v
                  WHERE v.screen='order_line_items' AND v.field_key=fl.lookup_key AND v.role='admin');

SELECT 'M3 全表带过来·lookup 生成数' AS report,
       related_module, count(*) AS lookups, count(DISTINCT target_field_key) AS 角色数
FROM field_lookups WHERE status='active' GROUP BY related_module;

SELECT 'M3 展示层' AS report, screen,
       count(*) AS 总字段, count(*) FILTER (WHERE visible) AS 默认展示,
       count(*) FILTER (WHERE NOT visible) AS 带过来但不展示
FROM field_visibility_config GROUP BY screen;
