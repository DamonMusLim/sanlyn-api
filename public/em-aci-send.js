(function(){
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别加拿大线 shipping_plans 记录，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取加拿大线船期记录，当前填充率 未接入。"));return}
    var table=document.createElement("table"),body=document.createElement("tbody");
    [["订舱号",r.shipment_no],["SO号",r.so_no],["提单号",r.bl_no],["承运人",r.carrier_code],["船名航次",[r.vessel,r.voyage].filter(Boolean).join(" / ")],["航线",[r.pol,r.pod].filter(Boolean).join(" -> ")],["ETD/ETA",[r.etd,r.eta].filter(Boolean).join(" / ")],["收货地/目的地",[r.place_of_receipt,r.final_destination].filter(Boolean).join(" / ")],["发货人",r.shipper_name],["收货人",r.consignee_name],["通知人",r.notify_name],["货物明细行",r.line_count?"真实记录 "+r.line_count:"未接入"]].forEach(function(pair){
  ...
  function renderFieldList(id,fields,emptyMsg){
    var box=$(id);clear(box);
    if(!fields.length){box.appendChild(el("div","empty",emptyMsg));return}
  ...
  function renderCoverage(){
    ...
    renderFieldList("coverage",fields,"未接入 · 缺字段清单，当前填充率 未接入。");
    var line=state.lineSummary||{};
    text($("lineCount"),line.total_rows?"明细 "+line.total_rows:"未接入");
    renderFieldList("lineFields",line.fields||[],"未接入 · 缺 customs_shipment_lines 或无法关联当前船期，当前填充率 未接入。");
  }
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺加拿大eManifest/ACI接口、通道凭证、CCN、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"em_aci_send_status / em_aci_sent_at / em_aci_receipt_no / em_aci_ccn / em_aci_filer_code")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("list").addEventListener("click",function(e){var b=e.target.closest(".row");if(!b)return;var id=b.dataset.id;state.selected=state.rows.find(function(r){return String(r.id)===String(id)})||state.selected;load(id)});
  $("reload").addEventListener("click",function(){load()});
  $("search").addEventListener("keydown",function(e){if(e.key==="Enter")load()});
  $("sendBtn").addEventListener("click",function(){alert("EM&ACI通道尚未对接：缺加拿大eManifest/ACI接口、通道凭证、em_aci_send_status / em_aci_sent_at / em_aci_receipt_no / em_aci_ccn 等字段；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"EM&ACI发送",url:location.pathname},location.origin);
  load();
})();
