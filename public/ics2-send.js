(function(){
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别欧线 shipping_plans 记录，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取欧线船期记录，当前填充率 未接入。"));return}
    var table=document.createElement("table"),body=document.createElement("tbody");
    [["订舱号",r.shipment_no],["SO号",r.so_no],["提单号",r.bl_no],["船公司",r.carrier_code],["船名航次",[r.vessel,r.voyage].filter(Boolean).join(" / ")],["航线",[r.pol,r.pod].filter(Boolean).join(" -> ")],["ETD/ETA",[r.etd,r.eta].filter(Boolean).join(" / ")],["收货地/目的地",[r.place_of_receipt,r.final_destination].filter(Boolean).join(" / ")],["发货人",r.shipper_name],["发货人EORI",r.shipper_eori],["收货人",r.consignee_name],["收货人EORI",r.consignee_eori],["通知人",r.notify_name],["货物明细行",r.line_count?"真实记录 "+r.line_count:"未接入"]].forEach(function(pair){
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺ICS2/ENS接口、申报方EORI、发送状态字段和MRN/LRN回执字段。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"ics2_send_status / ics2_sent_at / ics2_lrn / ics2_mrn / ics2_filer_eori")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("sendBtn").addEventListener("click",function(){alert("ICS2通道尚未对接：缺ICS2/ENS接口、申报方EORI、ics2_send_status / ics2_sent_at / ics2_lrn / ics2_mrn 等字段；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"ICS2(欧线)",url:location.pathname},location.origin);
  load();
})();
