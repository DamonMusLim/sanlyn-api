(function(){
  "use strict";
  var API="/api/db/shenzhen-nansha-manifest-send";
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别深圳/南沙口岸 customs_shipments 记录；需要 pol/place_of_issue/shipping_agent 中出现 SHENZHEN/SHEKOU/YANTIAN/CHIWAN/NANSHA/CNSZX/CNSHK/CNYTN/CNNAN/CNNAS/深圳/蛇口/盐田/赤湾/南沙，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取深圳/南沙舱单记录，当前填充率 未接入。"));return}
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺深圳/南沙口岸舱单申报接口、通道凭证、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","当前页面只用于核对深圳/南沙口岸舱单抬头、收发通和明细字段是否齐全，为将来发送做准备。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"shenzhen_nansha_manifest_status / shenzhen_nansha_manifest_sent_at / shenzhen_nansha_manifest_receipt_no")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("sendBtn").addEventListener("click",function(){alert("深圳/南沙舱单发送通道尚未对接：缺口岸申报接口、通道凭证、shenzhen_nansha_manifest_status / shenzhen_nansha_manifest_sent_at / shenzhen_nansha_manifest_receipt_no 三列；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"深圳/南沙-舱单发送",url:location.pathname},location.origin);
  load();
})();
