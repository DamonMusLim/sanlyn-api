(function(){
  "use strict";
  var API="/api/db/xiamen-manifest-send";
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别厦门口岸 customs_shipments 记录；需要 pol/place_of_issue/shipping_agent 中出现 XIAMEN/XMN/CNXMN/厦门，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取厦门舱单记录，当前填充率 未接入。"));return}
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺厦门口岸舱单申报接口、通道凭证、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","当前页面只用于核对厦门口岸舱单抬头、收发通和明细字段是否齐全，为将来发送做准备。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"xiamen_manifest_status / xiamen_manifest_sent_at / xiamen_manifest_receipt_no")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"厦门-舱单发送",url:location.pathname},location.origin);
  load();
})();
