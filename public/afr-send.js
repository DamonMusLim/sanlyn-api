(function(){
  ...
  function renderMetrics(){
    ...
    text($("mSend"),"未接入");
    $("summary").textContent="版本 "+VERSION+" · 生成时间 "+new Date(state.generatedAt||Date.now()).toLocaleString("zh-CN");
  }
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别日本线 shipping_plans 记录，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取日本线船期记录，当前填充率 未接入。"));return}
  ...
  function renderCoverage(){
    ...
    var fields=[].concat((state.coverage&&state.coverage.fields)||[],(state.coverage&&state.coverage.party_fields)||[],(state.lineSummary&&state.lineSummary.fields)||[]);
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺日本AFR/NACCS或第三方申报通道接口、通道凭证、申报方代码、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","当前页面只用于核对日本线船期、收发通和舱单明细字段是否齐全，为将来发送做准备。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"afr_send_status / afr_sent_at / afr_receipt_no / afr_filer_code / afr_filing_type")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("sendBtn").addEventListener("click",function(){alert("AFR发送通道尚未对接：缺日本AFR/NACCS或第三方申报接口、通道凭证、afr_send_status / afr_sent_at / afr_receipt_no / afr_filer_code / afr_filing_type 等字段；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"AFR发送",url:location.pathname},location.origin);
  load();
})();
