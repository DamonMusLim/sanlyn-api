(function(){
  "use strict";
  var API="/api/db/vgm-send";
  var state={rows:[],selected:null,containers:[],coverage:null,send:null};
  ...
  async function api(q,id){
    var qs=[];if(q)qs.push("q="+encodeURIComponent(q));if(id)qs.push("id="+encodeURIComponent(id));
    var r=await fetch(API+(qs.length?"?"+qs.join("&"):""),{headers:headers()});
  ...
  function renderMetrics(){
    ...
    text($("mCtn"),fieldRate(cov.container_fields));
    text($("mSend"),"未接入");
  ...
  function renderList(){
    ...
    state.rows.forEach(function(r){
      ...
      b.appendChild(el("span","",([r.carrier_code,r.vessel,r.voyage].filter(Boolean).join(" / ")||"未设置")+" · BL "+(r.bl_no||"未设置")+" · VGM "+(r.vgm_ready_rows||"未接入")+"/"+(r.container_rows||"未接入")));
      box.appendChild(b);
    });
  }
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取船期记录，当前填充率 未接入。"));return}
    var table=document.createElement("table"),body=document.createElement("tbody");
    [["订舱号",r.shipment_no],["SO号",r.so_no],["提单号",r.bl_no],["船公司",r.carrier_code],["船名航次",[r.vessel,r.voyage].filter(Boolean).join(" / ")],["航线",[r.pol,r.pod].filter(Boolean).join(" → ")],["ETD",r.etd],["截VGM",r.vgm_cutoff],["箱型/柜量",[r.container_type,r.container_qty].filter(Boolean).join(" / ")]].forEach(function(pair){
    ...
    text($("readyPill"),r.missing_count?"待补字段":"船期已填");
  }
  function renderContainers(){
    var box=$("containers");clear(box);
    if(!state.containers.length){box.appendChild(el("div","empty","未接入 · 缺 container_bookings 柜级记录或当前 BL 未匹配到柜，当前填充率 未接入。"));return}
    var table=document.createElement("table"),thead=document.createElement("thead"),hr=document.createElement("tr");
    ["柜号","封号","箱型","皮重kg","货重kg","VGM kg","磅单"].forEach(function(x){var th=document.createElement("th");text(th,x);hr.appendChild(th)});
    thead.appendChild(hr);table.appendChild(thead);
    var tb=document.createElement("tbody");
    state.containers.forEach(function(c){
      var tr=document.createElement("tr");
      [c.container_no,c.seal_no,c.container_type,c.tare_weight_kg,c.cargo_weight_kg,c.vgm_weight_kg,c.weight_ticket_url].forEach(function(v){td(tr,v)});
      tb.appendChild(tr);
    });
  ...
  function renderCoverage(){
    ...
    var fields=[].concat((state.coverage&&state.coverage.fields)||[],(state.coverage&&state.coverage.container_fields)||[]);
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺VGM发送通道接口、船公司/平台凭证、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","当前页面只用于核对船期、柜号、重量和磅单字段是否齐全，为将来发送做准备。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"vgm_send_status / vgm_sent_at / vgm_receipt_no")+"。当前填充率："+(rates||"未接入")+"。"));
    box.appendChild(el("p","muted","未接入条目不提供忽略，也不执行对外发送。"));
  }
  function render(){renderMetrics();renderList();renderDetail();renderContainers();renderCoverage();renderSendState()}
  async function load(id){
    try{
      var d=await api($("search").value.trim(),id);
      state.rows=d.data||[];state.selected=d.selected||state.rows[0]||null;state.containers=d.containers||[];
      state.coverage=d.coverage||null;state.send=d.send_channel||null;state.generatedAt=d.generated_at;render();
  ...
  $("sendBtn").addEventListener("click",function(){alert("VGM发送通道尚未对接：缺VGM发送接口、船公司/平台凭证、vgm_send_status / vgm_sent_at / vgm_receipt_no 三列；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"VGM发送",url:location.pathname},location.origin);
  load();
})();
