(function(){
  "use strict";
  var API="/api/db/tianjin-dalian-manifest-send";
  ...
  function renderList(){
    ...
    if(!state.rows.length){box.appendChild(el("div","empty","未接入 · 缺可识别天津/大连口岸 customs_shipments 记录；需要 pol/place_of_issue/shipping_agent 中出现 TIANJIN/XINGANG/TSN/TXG/CNTSN/CNTXG/天津/新港/DALIAN/DLC/CNDLC/大连，当前填充率 未接入。"));return}
  ...
  function renderDetail(){
    ...
    if(!r){box.appendChild(el("div","empty","未接入 · 缺可读取天津/大连舱单记录，当前填充率 未接入。"));return}
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺天津/大连口岸舱单申报接口、通道凭证、发送状态字段和回执字段。"));
    box.appendChild(el("p","muted","当前页面只用于核对天津/大连口岸舱单抬头、收发通和明细字段是否齐全，为将来发送做准备。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"tjdl_manifest_status / tjdl_manifest_sent_at / tjdl_manifest_receipt_no")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("sendBtn").addEventListener("click",function(){alert("天津/大连舱单发送通道尚未对接：缺口岸申报接口、通道凭证、tjdl_manifest_status / tjdl_manifest_sent_at / tjdl_manifest_receipt_no 三列；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"天津/大连-舱单",url:location.pathname},location.origin);
  load();
})();
