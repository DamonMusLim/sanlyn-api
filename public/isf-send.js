(function(){
  ...
  function renderDetail(){
    ...
    [["订舱号",r.shipment_no],["SO号",r.so_no],["提单号",r.bl_no],["船公司",r.carrier_code],["船名航次",[r.vessel,r.voyage].filter(Boolean).join(" / ")],["航线",[r.pol,r.pod].filter(Boolean).join(" -> ")],["ETD/ETA",[r.etd,r.eta].filter(Boolean).join(" / ")],["进口商",r.importer_name],["进口商编号",r.importer_no],["收货人",r.consignee_name],["卖方/买方",[r.seller_name,r.buyer_name].filter(Boolean).join(" / ")],["制造商",r.manufacturer_name],["送达方",r.ship_to_name],["装箱地点",r.stuffing_location],["拼箱方",r.consolidator_name],["货物明细行",r.line_count?"真实记录 "+r.line_count:"未接入"]].forEach(function(pair){
  ...
  function renderFieldList(id,fields,emptyMsg){
    ...
    fields.forEach(function(f){
      ...
      else if(!Number(f.total||0))text(meta,"未接入 · 缺可统计记录；当前填充率 未接入");
  ...
  function renderSendState(){
    ...
    box.appendChild(el("p","muted",s.note||"缺ISF申报通道接口、Bond/申报方代码、发送状态字段和交易回执字段。"));
    box.appendChild(el("p","muted","缺字段："+(missing||"isf_send_status / isf_sent_at / isf_transaction_no / isf_bond_no / isf_filer_code")+"。当前填充率："+(rates||"未接入")+"。"));
  ...
  $("sendBtn").addEventListener("click",function(){alert("ISF发送通道尚未对接：缺ISF接口、通道凭证、isf_send_status / isf_sent_at / isf_transaction_no / isf_bond_no / isf_filer_code 等字段；当前页面只做数据核对。")});
  if(window.parent!==window)window.parent.postMessage({type:"sanlyn:module-ready",title:"ISF发送(美线)",url:location.pathname},location.origin);
  load();
})();
