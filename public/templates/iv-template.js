// iv-template.js — 商业发票动态加载。版式照 documents.js type=pi（灰白版），字段只对应不编造。
...
function init(){
  ...
  if(!orderNo){banner('err','未传 order_no，请加 ?order_no=XX&token=YY');document.getElementById('ivBody').innerHTML='';return;}
  var idsRaw=qp('ids');
  ...
  try{var s=JSON.parse(localStorage.getItem('iv_seal')||'null');if(s&&s.url)applySeal(s.url);}catch(e){}
}
...
function fillDoc(primary,all){
  ...
  // DETAILS — 编号铁律[[sanlyn_id_hierarchy_rule]]: No.=fs_no内部号(多订单只一个), PO No.=客户PO, 工厂contract_no绝不上客户单
  ...
  document.getElementById('ivBody').innerHTML=html;
  banner('','');
}
...
function applySeal(url){var img=document.getElementById('sellerSeal'),h=document.getElementById('sealHint');if(img){img.src=url;img.style.display='block';}if(h)h.style.display='none';try{localStorage.setItem('iv_seal',JSON.stringify({url:url}));}catch(e){}}
...
function saveDraft(){try{localStorage.setItem('iv_draft_'+(qp('order_no')||'manual'),JSON.stringify({html:document.getElementById('page').innerHTML}));banner('info','✓ 草稿已保存');setTimeout(function(){banner('','');},1500);}catch(e){}}
function downloadPng(){
  ...
  s.onload=function(){html2canvas(document.getElementById('page'),{scale:2,useCORS:true,backgroundColor:'#fff'}).then(function(c){document.querySelector('.toolbar').style.display='';var a=document.createElement('a');a.download='IV-'+(qp('order_no')||'draft')+'.png';a.href=c.toDataURL('image/png');a.click();btn.textContent='📥 下载图片';btn.disabled=false;}).catch(function(){document.querySelector('.toolbar').style.display='';btn.textContent='📥 下载图片';btn.disabled=false;});};
