(function(){
  ...
  function injectStyle(){
    ...
    style.textContent = `.ctb input,.ctb select,.ctb textarea{background:var(--card,var(--card,#fff));color:var(--ink,var(--ink,#111));border:1px solid var(--line,var(--line,#c8cdd6))}.ctb input::placeholder,.ctb textarea::placeholder{color:var(--faint,var(--faint,#9ca3af))}.ctb input[type=file]{color:var(--sub,var(--sub,#6b7280))}/*__THEMED__ __THEMED2__*/
.ctb .card{background:var(--card,var(--card,#fff));border-radius:10px;border:1px solid var(--line,var(--line,#e0e4ea));overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);margin-bottom:14px}
.ctb .section{padding:14px 16px;border-bottom:1px solid var(--ground,#f0f2f5)}.ctb .section:last-child{border-bottom:none}
.ctb .grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px}.ctb .form-field{margin-bottom:10px}
.ctb .form-label{font-size:11px;font-weight:700;color:var(--ink,var(--ink,#374151));margin-bottom:4px;display:block}.ctb .form-label .req{color:var(--err,var(--err,#dc2626))}
.ctb .form-input{width:100%;border:1.5px solid var(--line,#c8cdd6);border-radius:6px;padding:8px 10px;font-size:13px;outline:none;font-family:inherit;background:var(--card,var(--card,#fff));color:var(--ink,var(--ink,#111))}
...
.ctb .btn{padding:7px 16px;border-radius:7px;border:none;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit}.ctb .btn-green{background:var(--ok,#059669);color:var(--card,#fff)}.ctb .btn-ghost{background:var(--row,var(--row,#f3f4f6));color:var(--ink,var(--ink,#374151));border:1px solid var(--line,var(--line,#e0e4ea))}.ctb .btn-sm{padding:5px 12px;font-size:11px}.ctb .btn:disabled{opacity:.5;cursor:not-allowed}
.ctb .badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:800}.ctb .badge-red{background:var(--err-soft,#fee2e2);color:var(--err,var(--err,#b91c1c));border:1px solid var(--err,#fca5a5)}
.ctb .step-card{background:var(--card,var(--card,#fff));border-radius:10px;border:1px solid var(--line,var(--line,#e0e4ea));margin-bottom:14px;overflow:hidden;transition:border-color .2s}.ctb .step-card.required{border-color:var(--wait,#fbbf24)}.ctb .step-card.done{border-color:#86efac}
.ctb .step-head{padding:12px 16px;display:flex;align-items:center;justify-content:space-between;background:var(--card,var(--card,#fff));border-bottom:1px solid var(--ground,#f0f2f5);transition:background .15s;user-select:none}.ctb .step-card.done .step-head{cursor:pointer}.ctb .step-card.done .step-head:hover{background:var(--row,var(--row,#f8fafc))}
.ctb .step-num{width:26px;height:26px;border-radius:50%;background:var(--ink,#1a1d23);color:var(--card,#fff);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex-shrink:0}.ctb .step-num.done{background:var(--ok,#059669)}
.ctb .step-title{font-size:13px;font-weight:800;color:var(--ink,var(--ink,#1a1d23))}.ctb .step-sub{font-size:11px;color:var(--sub,var(--sub,#6b7280));margin-top:1px}.ctb .step-summary{display:none;font-size:11px;font-weight:700;color:var(--ok,var(--ok,#059669));flex:1;margin:0 10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ctb .step-card.done .step-summary{display:block}.ctb .step-card.done .step-title{font-size:12px;color:var(--sub,var(--sub,#6b7280));font-weight:600}.ctb .step-card.done .step-sub{display:none}
.ctb .chevron{font-size:12px;color:var(--faint,var(--faint,#9ca3af));transition:transform .25s;flex-shrink:0}.ctb .step-card.done .chevron{transform:rotate(0deg)}.ctb .step-card:not(.done) .chevron{transform:rotate(180deg)}.ctb .step-body{overflow:hidden;transition:max-height .3s ease,opacity .3s ease}.ctb .step-card.done .step-body{max-height:0!important;opacity:0}.ctb .step-card:not(.done) .step-body{max-height:2000px;opacity:1}
.ctb .ctn-group{border:1.5px solid var(--line,#e0e4ea);border-radius:8px;margin-bottom:10px;overflow:hidden}.ctb .ctn-group-head{background:var(--row,var(--row,#f8fafc));padding:7px 12px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line,#e0e4ea)}.ctb .ctn-group-title{font-size:11px;font-weight:800;color:var(--ink,var(--ink,#374151))}
.ctb .chip{background:var(--card,var(--card,#fff));border:1px solid var(--line,var(--line,#e0e4ea));border-radius:8px;padding:8px 14px;font-size:12px}.ctb .chip b{color:var(--ink,var(--ink,#1a1d23))}.ctb .chip span{color:var(--sub,var(--sub,#6b7280))}.ctb .qty-btn{border:1px solid var(--line,var(--line,#c8cdd6));background:var(--card,var(--card,#fff));border-radius:5px;width:24px;height:24px;font-weight:800;cursor:pointer}.ctb .qty-in{width:42px;border:1px solid var(--line,var(--line,#c8cdd6));border-radius:5px;padding:4px;text-align:center;font-weight:800}
.ctb .photo-grid{display:flex;flex-wrap:wrap;gap:6px;padding:8px;background:var(--card,#fafafa);border:1px solid #eef2f7;border-radius:8px}.ctb .photo-cell{position:relative;width:58px;height:58px;border-radius:6px;overflow:hidden;border:1px solid var(--line,var(--line,#e0e4ea));background:var(--row,#f1f5f9);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;font-size:10px;color:var(--sub,var(--sub,#64748b));text-align:center}.ctb .photo-cell img{width:100%;height:100%;object-fit:cover}.ctb .photo-cell.add{border-style:dashed;border-color:#cbd5e1}.ctb .photo-cell .del-btn{position:absolute;top:1px;right:1px;width:16px;height:16px;background:var(--err,#dc2626);color:var(--card,#fff);border-radius:50%;font-size:11px;font-weight:800;line-height:16px}
.ctb .save-status{text-align:center;font-size:11px;color:var(--faint,var(--faint,#9ca3af));margin:8px 0 14px}@media(max-width:480px){.ctb .grid2,.ctb .veh-grid{grid-template-columns:1fr!important}}
`;
  ...
  function photoGrid(i,ps){ ps = ps || []; return ps.map((p,j)=>{ const u = photoUrl(p), img = (p.mime || "").startsWith("image"); return `<div class="photo-cell">${img && u ? `<img src="${esc(u)}" alt="">` : "📄<br>" + esc(p.filename || "文件")}<div class="del-btn" onclick="CollabTruckBlock.delPhoto(${i},${j})">×</div></div>`; }).join("") + `<div class="photo-cell add" onclick="CollabTruckBlock.pickPhoto(${i})"><div><div style="font-size:18px;">＋</div>照片/磅单</div></div>`; }
	  function vehHtml(i,d={}){
	    ...
	    const src = d.loading_source ? `取自${d.loading_source}，可修改` : "";
    return `<div class="ctn-group" id="veh_${i}"><div class="ctn-group-head"><div class="ctn-group-title">🚛 车 ${i+1}</div>${state.vehs.length > 1 ? `<span style="cursor:pointer;color:var(--err,var(--err,#dc2626));font-size:11px;font-weight:700;" onclick="CollabTruckBlock.delVeh(${i})">× 删除</span>` : ""}</div>
    <div class="veh-grid" style="padding:12px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ...
      <div class="form-field" style="margin:0;"><label class="form-label">装柜预约时间(工厂/车队可改)</label><input class="form-input" id="loading_time_${i}" type="datetime-local" value="${esc(dtLocal(loadingTime))}"></div>
	      <div class="form-field" style="margin:0;"><label class="form-label">工厂提货地址</label><div style="font-size:10.5px;color:${addr ? "var(--sub)" : "var(--faint)"};margin-bottom:4px;">📍 ${esc(addr || "提货地址待补")}${src ? ` · ${esc(src)}` : ""}</div><input class="form-input" id="loadaddr_${i}" value="${esc(addr)}" placeholder="📍 提货地址待补"></div>
	      <div class="form-field" style="margin:0;"><label class="form-label">工厂联系人</label><div style="font-size:10.5px;color:${contact ? "var(--sub)" : "var(--faint)"};margin-bottom:4px;">厂内联系人：${esc(contact || "待补")}${src ? ` · ${esc(src)}` : ""}</div><input class="form-input" id="loadcontact_${i}" value="${esc(contact)}" placeholder="厂内联系人待补"></div>
