(function(){
  ...
  function validNo(v){v=String(v||'').trim();return v&&v!=='—'?v:'';}
  function docSeed(s){return validNo(s&&s.bl_no)||validNo(s&&s.sc_no)||sid();}
  function blockedMsg(){var d=state&&state.data;if(!d)return '';return d.needs_docno?(d.docno_warning||'本票缺提单号和合同号,无法生成对外单号,请先补齐'):(d.needs_amount?(d.amount_warning||'本票未录运费,请先在票上录入后再出单'):'');}
  function pdfBlocked(){return !!blockedMsg();}
  function pdfUrl(){
  ...
  function party(d){
    if(page==='portcharge'){
      var f=d.factory||{};
      return {label:'TO (工厂名称):',name:f.name_cn||'',addr:f.address||'地址 Address: _______________________________'};
    }
    var c=d.customer||{};
  ...
  function chargeHtml(d){
    ...
    var rows=(d.charges||[]).map(function(r){var amountOnly=r.qty==null||r.unit_price==null;return '<tr><td>'+h(r.cost_category||'')+'</td><td>'+h(r.charge_basis||'整票')+'</td><td class="c">'+h(r.currency||'CNY')+'</td><td class="c">'+(amountOnly?'':money(r.qty,0))+'</td><td class="r">'+(amountOnly?'':money(r.unit_price))+'</td><td class="r">'+money(r.amount)+'</td></tr>';}).join('');
  ...
  function render(d){
    ...
    document.title=(page==='portcharge'?'Port Charge Statement':'Freight Invoice')+' · '+docSeed(s);
    window._docBaseName=(page==='portcharge'?'PC-':'FI-')+docSeed(s);
    var seller=d.seller||{},sellerName=seller.name_en||"SHANGHAI OCEAN BABY INT'L LOGISTICS CO., LTD.",sellerAddr=seller.address_en||'';
    var block=blockedMsg();
    var warn=block?'<div class="warn-card">'+h(block)+'</div>':(d.used_fallback_card?'<div class="warn-card">系统未查到该票真实账单明细，以下为标准参考费率估算，非实际账单数据，出单前请人工核实真实费用！<br>NOT ACTUAL BILLED CHARGES — verify against real supplier invoice before issuing.</div>':(d.needs_terms?'<div class="warn-card">该票未填贸易条款，系统已停止生成标准费率兜底卡；请先补齐贸易条款。</div>':''));
    warn+=(d.seller_fallback?'<div class="warn-card">⚠ 销方抬头取数失败,当前为兜底文案,请核对后再出单</div>':'');
    return '<div class="freight-doc" id="freightPage"><div class="hdr"><div><div class="co-en">'+h(sellerName)+'</div><div class="co-cn">上海洋宝宝国际物流有限公司</div>'+(sellerAddr?'<div class="co-cn">'+h(sellerAddr)+'</div>':'')+'<div class="tag">Ocean Freight · Air Freight · Express · Integrated Logistics Solutions</div></div><div><div class="doc-en">INVOICE</div><div class="doc-cn">'+docCn+'</div><div class="inv-no">No. '+h(d.doc_no)+'</div></div></div>'+warn+'<div class="info-grid"><div class="info-box"><div class="row"><div class="lbl">'+p.label+'</div><div class="val big">'+h(p.name)+'<div style="font-size:9px;font-weight:400;color:#555;margin-top:2px">'+h(p.addr)+'</div></div></div><div class="row"><div class="lbl">SHPT MODE:</div><div class="val">Sea Export</div></div><div class="row"><div class="lbl">INV/BL NO.:</div><div class="val">'+h(s.bl_no)+'</div></div><div class="row"><div class="lbl">DATE (出单日期):</div><div class="val">'+h(s.gen_date)+'</div></div></div><div class="info-box"><div class="row"><div class="lbl">Vessel/Voyage (船名航次):</div><div class="val">'+h(s.vessel)+'</div></div><div class="row"><div class="lbl">ETD (离港日):</div><div class="val">'+h(date(s.etd))+'</div></div><div class="row"><div class="lbl">P.O.L (起运港):</div><div class="val">'+h(s.pol)+'</div></div><div class="row"><div class="lbl">P.O.D (目的港):</div><div class="val">'+h(s.pod)+'</div></div></div></div>'+containerHtml(d.containers)+chargeHtml(d)+payHtml(d)+'<div class="bottom"><div class="box"><div class="box-title">TERMS &amp; CONDITIONS (法律声明与条款)</div>1. PAYMENT DUE: Please arrange payment strictly within the agreed credit term. Late payment may result in delayed release of the Bill of Lading or cargo.<br>2. EXCHANGE RATE: For USD charges settled in RMB, the exchange rate shall be subject to our company\'s notification.<br>3. LIABILITY: All business is transacted under our Standard Trading Conditions.</div><div class="box"><div class="box-title">BANKING INFORMATION (银行信息)</div>Bank Name: <strong>BANK OF CHINA XIAMEN BRANCH</strong><br>Account Name: <strong>SHANGHAI OCEAN BABY INTERNATIONAL LOGISTICS CO., LTD.</strong><br>Swift Code: <strong>BKCHCNBJ73A</strong><br>Bank Addr: No. 40 North Hubin Road, Xiamen<br>USD Account (美金账号): <strong>433849630299</strong><br>CNY Account (人民币账号): <strong>433849860868</strong><br><span style="color:#c00;font-size:8px">* Please check the account number carefully before remittance.</span></div></div><div class="sigs"><div class="sig"><div class="sig-t">BUYER AUTHORIZED SIGNATURE<span class="seal-wrap" id="freight-buyer-seal-area" ondragover="event.preventDefault()" ondrop="dropSeal(event,\'freight:buyer\')"><img id="freight-buyer-seal"></span><span class="seal-hint" id="freight-buyer-seal-hint" onclick="openSealPicker(\'freight:buyer\')">click seal</span></div><div class="sig-sub">(Signature / Company Seal)</div></div><div class="sig"><div class="sig-t">SELLER AUTHORIZED SIGNATURE<span class="seal-wrap" id="freight-seller-seal-area" ondragover="event.preventDefault()" ondrop="dropSeal(event,\'freight:seller\')"><img id="freight-seller-seal"></span><span class="seal-hint" id="freight-seller-seal-hint" onclick="openSealPicker(\'freight:seller\')">click seal</span></div><div class="sig-sub">(Signature / Company Seal)</div></div></div></div>';
  }
  window._docUrl=function(fmt){return fmt==='pdf'?pdfUrl():location.href;};
  window.dlDoc=function(fmt){if(fmt==='xlsx')return downloadXlsx();var msg=blockedMsg();if(msg){banner('err',msg);return;}window.open(pdfUrl(),'_blank');};
