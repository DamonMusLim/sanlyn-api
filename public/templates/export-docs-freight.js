(function(){
  ...
  function lang(){return String(qp('lang')||localStorage.getItem('statement_portal_lang')||'en').toLowerCase().replace(/^zh.*/,'zh').replace(/^ms.*/,'ms');}
  function i18nDict(){
    return window.STATEMENT_PORTAL_I18N||window.statementPortalI18n||window.STATEMENT_I18N||window.statementPortalI18N||null;
  }
  var FALLBACK_EN={
    portcharge_title:'Port Charge Statement',invoice:'INVOICE',invoice_breakdown:'Invoice Breakdown',
    taxed_port_charge:'Taxable Port Charge Agency Fee',freight_forwarding_service:'International Freight Forwarding Service',
    tax_rate:'Tax Rate',tax_included:'Tax Included',tax_exempt:'Tax Exempt',pending_taxed:'Taxable amount pending',
    port_charges_payable:'Port charges are payable in CNY only.',total_payable_cny:'TOTAL PAYABLE IN CNY',
    port_charges_only:'Port charges only · Remit to CNY A/C below'
  };
  function tr(k){
    var dict=i18nDict(),l=lang(),pack=dict&&(dict[l]||dict.en);
    return (pack&&(pack[k]||pack.portcharge&&pack.portcharge[k]))||FALLBACK_EN[k]||k;
  }
  function loadStatementI18n(){
    if(i18nDict())return Promise.resolve();
    return new Promise(function(resolve){
      var s=document.createElement('script');
      s.src='/statement-portal-i18n.js';
  ...
  function payHtml(d){
    if(page==='portcharge')return '<div class="fx-note">* '+h(tr('port_charges_payable'))+'</div><div class="pay-grid"><div class="pay-box cny" style="grid-column:1/-1"><div class="plbl">'+h(tr('total_payable_cny'))+'</div><div class="pamt">¥ '+money(d.totals.cny)+'</div><div class="psub">'+h(tr('port_charges_only'))+'</div></div></div>';
  ...
  function breakdownHtml(d){
    if(page!=='portcharge')return '';
    var x=d.invoice_split||{},taxed=Number(x.taxed_amount)||0,rate=Number(x.taxed_rate)||0;
    var source=x.source==='待填'?' <span class="split-note">'+h(tr('pending_taxed'))+'</span>':'';
    return '<div class="split-box"><div class="split-title">开票金额 / '+h(tr('invoice_breakdown'))+source+'</div><table><thead><tr><th>Item</th><th class="c">'+h(tr('tax_rate'))+'</th><th class="r">Amount</th><th class="r">Invoice Amount</th></tr></thead><tbody><tr><td>'+h(tr('taxed_port_charge'))+'</td><td class="c">'+money(rate*100,0)+'%</td><td class="r">¥ '+money(taxed)+'</td><td class="r">¥ '+money(x.taxed_with_tax)+'</td></tr><tr><td>'+h(tr('freight_forwarding_service'))+'</td><td class="c">'+h(tr('tax_exempt'))+'</td><td class="r">¥ '+money(x.free_amount)+'</td><td class="r">¥ '+money(x.free_amount)+'</td></tr></tbody></table></div>';
  }
  function render(d){
    var p=party(d),s=d.shipment,docCn=page==='portcharge'?tr('portcharge_title'):'运费发票';
    document.title=(page==='portcharge'?tr('portcharge_title'):'Freight Invoice')+' · '+(s.shipment_no||s.bl_no||sid());
  ...
  window.toggleLang=function(){
    var order=['en','ms','zh'],next=order[(order.indexOf(lang())+1)%order.length];
    localStorage.setItem('statement_portal_lang',next);
    if(state&&state.data)document.getElementById('freightRoot').innerHTML=render(state.data);
    banner('info','Language: '+next.toUpperCase());setTimeout(function(){banner('','');},1200);
  };
  ...
  window.downloadPng=function(){
    ...
    var pageEl=document.getElementById('freightPage');if(pageEl)pageEl.classList.add('exporting');
    var done=function(){document.querySelector('.toolbar').style.display='';if(pageEl)pageEl.classList.remove('exporting');btn.textContent='📥 下载图片';btn.disabled=false;};
  ...
  window.initFreightDocs=function(){
    ...
    loadStatementI18n().then(function(){
      return fetch(API+'/api/db/shipping-plan-doc-data?id='+encodeURIComponent(sid())+'&page='+encodeURIComponent(page),{headers:authH()});
    }).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error(j.error||r.status);return j;});}).then(function(j){state=j;root.innerHTML=render(j.data);banner('','');}).catch(function(e){banner('err',e.message);});
  };
