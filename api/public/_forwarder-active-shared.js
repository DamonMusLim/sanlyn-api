...
export async function loadSupplierPortCharges(pool, companyId, supplierName){
  const { rows } = await pool.query(
    `WITH candidate AS (
       SELECT sp.id AS sp_id, sp.carrier_code, sp.container_type, sp.etd,
     ...
     SELECT p.sp_id, p.carrier_code, p.container_type, p.etd, p.bill_id, p.bl_no,
       ...
       LEFT JOIN LATERAL (
         SELECT standard_item_code, standard_item_name, unit_basis, include_in_baseline
          ...
          WHERE i.raw_item_name = p.cost_category
...
export function attachSupplierPortCharges(plans, billRows){
  ...
  (billRows || []).forEach(function(row){
    ...
    var chargeBasis = cleanText(row.charge_basis).toLowerCase();
    if (chargeBasis !== "per_container") {
      logUnmappedPortCharge(feeName, blNo, "charge_basis=" + (chargeBasis || "blank"));
    ...
    var slot = target.byGroup[group] || (target.byGroup[group] = { total:0, src_plan_id:row.sp_id });
    slot.total += amount;
  });
}
...
export function addSupplierPortCharge(carrier, row, ct, tk){
  ...
  if (charge && isUsablePortCharge(charge.total)) {
    var existing = carrier.charges[group];
    if (!existing || tk >= existing.t) {
      carrier.charges[group] = { t:tk, total:charge.total, src_plan_id:charge.src_plan_id };
    }
  } else if (charge) {
