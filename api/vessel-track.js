/**
 * Vercel Serverless Function - 4portun API 代理
 * 环境变量: PORTUN_APP_ID, PORTUN_SECRET
 */
...
function normalizeEvent(e) {
  ...
  return {
    code: e.eventCode, label: meta.label, icon: meta.icon,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = {
      ...
      etd: raw.etd || '', eta: raw.eta || '',
      atd: raw.atd || '', ata: raw.ata || '',
      currentPosition: (raw.currentLat && raw.currentLng)
        ? { lat: parseFloat(raw.currentLat), lng: parseFloat(raw.currentLng) } : null,
      events,
