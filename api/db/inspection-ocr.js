...
export async function runInspectionOcr(pool, body = {}) {
  ...
  const dry_run = body.dry_run === true || body.dry_run === "true" || body.dry_run === "1" || !confirmed;
  ...
  let ordPreview = null;
  if (orderNo) {
    const r = await pool.query(`SELECT id, order_no, contract_no, factory_code FROM orders WHERE order_no=$1 LIMIT 1`, [orderNo]);
    ordPreview = r.rows[0] || null;
  }
  if (!ordPreview && contractNo) {
    const r = await pool.query(`SELECT id, order_no, contract_no, factory_code FROM orders WHERE contract_no=$1 ORDER BY id LIMIT 1`, [contractNo]);
    ordPreview = r.rows[0] || null;
  }
  ...
  const plan = {
    ...
    matched_order: ordPreview ? ordPreview.order_no : null,
    inspection_sheet: ordPreview ? "upsert inspection_request_sheets" : "no order matched, no sheet write",
    company_registration_no: ordPreview && ordPreview.factory_code && prodRegPreview
      ? { company_code: ordPreview.factory_code, value: prodRegPreview, write_if_empty: true }
      : null,
  };
  if (dry_run) return { success: true, dry_run: true, confirmed: false, confirmation_required: true, plan, parsed };
