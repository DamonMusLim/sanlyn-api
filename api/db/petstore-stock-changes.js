...
async function listRows(req) {
  ...
  const params = [storeCode, productCode, from, to, pageSize, offset];
  const sql = `
    WITH filtered AS (
      SELECT product_code, product_name, spec, category_name, order_type, order_no,
             stock_before, stock_after, delta, change_time, remark, store_code
        FROM petstore_stock_ledger
       WHERE ($1::text IS NULL OR store_code = $1)
         AND ($2::text IS NULL OR product_code = $2)
         AND ($3::timestamptz IS NULL OR change_time >= $3::timestamptz)
         AND ($4::timestamptz IS NULL OR change_time <= $4::timestamptz)
    ), total_count AS (
      ...
      SELECT product_code, product_name, spec, category_name, order_type, order_no,
             stock_before, stock_after, delta, change_time, remark, store_code
        FROM filtered
       ORDER BY change_time DESC NULLS LAST, product_code
       LIMIT $5 OFFSET $6
    )
    SELECT COALESCE(
