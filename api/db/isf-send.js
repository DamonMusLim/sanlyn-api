// /api/db/isf-send — US ISF declaration-channel read lens. Read-only, no external send.
...
const PARTY_FIELDS = [
  ["importer_name", "进口商"], ["importer_no", "进口商编号"], ["consignee_name", "收货人"],
  ["seller_name", "卖方"], ["buyer_name", "买方"], ["manufacturer_name", "制造商"],
  ["ship_to_name", "送达方"], ["stuffing_location", "装箱地点"], ["consolidator_name", "拼箱方"],
];
const LINE_FIELDS = [
  ["declaration_name", "申报品名"], ["hs_code", "HTS/HS编码"], ["origin_country", "原产国"],
  ["ctns", "箱数"], ["gw_kg", "毛重"],
];
const SEND_FIELDS = [
  ["isf_send_status", "ISF发送状态"], ["isf_sent_at", "ISF发送时间"],
  ["isf_transaction_no", "ISF交易号"], ["isf_bond_no", "Bond编号"],
  ["isf_filer_code", "申报方代码"],
];
...
function rowOut(row, planCols, customsCols) {
  ...
  return {
    ...
    container_type: row.container_type, container_qty: row.container_qty, importer_name: row.importer_name,
    importer_no: row.importer_no, consignee_name: row.consignee_name, seller_name: row.seller_name,
    buyer_name: row.buyer_name, manufacturer_name: row.manufacturer_name, ship_to_name: row.ship_to_name,
    stuffing_location: row.stuffing_location, consolidator_name: row.consolidator_name,
...
async function listPlans(pool, planCols, hasCustoms, customsCols, hasLines, q) {
  ...
  const lineAgg = hasLines && canJoin && customsCols.has("id") ? ", (SELECT COUNT(*)::int FROM customs_shipment_lines ln WHERE ln.shipment_id = cs.id) AS line_count" : ", NULL::int AS line_count";
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺ISF申报通道接口、通道凭证、Bond/申报方代码、发送状态字段和交易回执字段；本页不对外发送。",
  };
}
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      line_summary: await lineSummary(pool, hasLines, lineCols, selectedRow?.customs_id),
      coverage: {
