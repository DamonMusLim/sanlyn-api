...
async function handleFileProxy(req, res, pool) {
  ...
  if (type === "booking_note") {
    ...
    } catch (e) { return res.status(502).json({ ok: false, error: "托书服务不可用" }); }
  }
  if (type === "bl_sample") {
...
async function handleCollabUpload(req, res, pool) {
  ...
  if (auth.role === "supplier_portal" && /S\/?O|入货|排载|配舱|订舱确认|FCL|VGM|SI/i.test(`${safe} ${purpose || ""}`)) {
    const extracted = req.body.text || req.body.ocr_text || await extractTextFromUpload(buf, safe, mime);
    const text = String(extracted || "").slice(0, 200000);
