...
export async function renderBookingInstruction(ctx) {
  ...
  const orderNos = Array.isArray(_raw.orderNos) && _raw.orderNos.length ? _raw.orderNos : (sp.order_no ? [sp.order_no] : []);
  let shipper = { nameEN: "", nameCN: "", address: "", phone: "", contactName: "", email: "" };
  let issuing = "", orderIds = [];
  if (orderNos.length) {
  ...
  const consText = [consignee, [pick(_raw.consignee_contact, ""), pick(_raw.consignee_phone, ""), pick(_raw.consignee_country, "")].filter(Boolean).join(" · ")].filter(Boolean).join("\n");
  ...
  const cargoRowsHtml = (cargo.length ? cargo : [{}]).map(c => {
    const nameLine = [c.bl_description || "", c.declaration_name_en || ""].filter(Boolean).join(" ");
  ...
  }).join("") + (cargo.length ? `<tr class="total-row"><td></td><td><input type="text" value="${a(fmtNum(totQty) + " ctns")}" /></td><td style="text-align:right;font-weight:700">合计 TOTAL</td><td><input type="text" value="${a(fmtNum(totGw, 0) + " kgs")}" /></td><td><input type="text" value="${a(fmtNum(totCbm, 3) + " cbm")}" /></td></tr>` : "");
