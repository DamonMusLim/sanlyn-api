...
async function compressImageForOcr(imgBytes, filename, skipCompression) {
  const original = { imgBytes, media_type: mimeOf(filename) };
  if (skipCompression) return original;

  try {
    const meta = await sharp(imgBytes).rotate().metadata();
    const longSide = Math.max(Number(meta.width || 0), Number(meta.height || 0));
    if (imgBytes.length <= OCR_IMAGE_MAX_BYTES && longSide <= OCR_IMAGE_MAX_SIDE) {
      return original;
    }

    const compressed = await sharp(imgBytes)
      .rotate()
      .resize({
        width: OCR_IMAGE_MAX_SIDE,
        height: OCR_IMAGE_MAX_SIDE,
        fit: "inside",
        withoutEnlargement: true
      })
      .jpeg({ quality: 82 })
      .toBuffer();
    return { imgBytes: compressed, media_type: "image/jpeg" };
  } catch (err) {
    console.warn("[slip-core] image compression skipped:", err.message);
    return original;
  }
}
...
export async function callMiniMaxVision(fileBytes, filename, absPath, minimaxApiKey, promptText) {
  ...
  ({ imgBytes, media_type } = await compressImageForOcr(imgBytes, filename, isPdfConverted));
...
export async function ocrPaymentAdvice(fileBytes, filename, absPath, minimaxApiKey) {
  return callMiniMaxVision(fileBytes, filename, absPath, minimaxApiKey, OCR_PROMPT);
}

export async function dedupeAndRegisterUpload(pool, { domain, fileBytes, filename, storedPath, uploader, docType }) {
  ...
  const canonicalDoc = await registerCanonicalDocument(pool, {
    domain,
    doc_type: docType || "payment_advice",
