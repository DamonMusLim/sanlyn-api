// /api/db/xiamen-manifest-send — Xiamen manifest send readiness lens. Read-only.
...
const SEND_FIELDS = [
  ["xiamen_manifest_status", "厦门舱单发送状态"],
  ["xiamen_manifest_sent_at", "厦门舱单发送时间"],
  ["xiamen_manifest_receipt_no", "厦门舱单回执号"],
];
const XIAMEN_HINTS = ["%XIAMEN%", "%XMN%", "%CNXMN%", "%厦门%"];
...
async function listShipments(pool, cols, hasContainers, hasLines, q) {
  ...
  if (search) {
    ...
    const cond = xiamenCond(cols, params);
    if (cond) conds.push(cond);
  }
  if (q.id && cols.has("id")) {
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺厦门口岸舱单申报通道、通道凭证、发送状态字段和回执字段；本页不对外发送。",
  };
}
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[xiamen-manifest-send]", err);
