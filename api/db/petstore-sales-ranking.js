...
async function listRows(req) {
  ...
  const velocityTier = cleanText(req.query?.velocity_tier, 80);
  const params = [storeCode, category, velocityTier, pageSize, offset];
  const sql = `
    WITH filtered AS (
      SELECT product_code, product_name, category_name, spec, qty_30, qty_90,
             daily_avg_30, cur_stock, days_of_supply, velocity_tier
        FROM public.petstore_sku_sales_dna
       WHERE ($1::text IS NULL OR store_code = $1)
         AND ($2::text IS NULL OR category_name = $2)
         AND ($3::text IS NULL OR velocity_tier = $3)
    ), total_count AS (
      ...
      SELECT product_code, product_name, category_name, spec, qty_30, qty_90,
             daily_avg_30, cur_stock, days_of_supply, velocity_tier
        FROM filtered
       ORDER BY qty_30 DESC NULLS LAST, product_code
       LIMIT $4 OFFSET $5
    )
    SELECT COALESCE(
