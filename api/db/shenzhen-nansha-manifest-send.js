// /api/db/shenzhen-nansha-manifest-send — Shenzhen/Nansha manifest readiness lens. Read-only.
...
const SEND_FIELDS = [
  ["shenzhen_nansha_manifest_status", "深圳/南沙舱单发送状态"],
  ["shenzhen_nansha_manifest_sent_at", "深圳/南沙舱单发送时间"],
  ["shenzhen_nansha_manifest_receipt_no", "深圳/南沙舱单回执号"],
];
const PORT_HINTS = [
  "%SHENZHEN%", "%SHEKOU%", "%YANTIAN%", "%CHIWAN%", "%NANSHA%",
  "%CNSZX%", "%CNSHK%", "%CNYTN%", "%CNNAN%", "%CNNAS%",
  "%深圳%", "%蛇口%", "%盐田%", "%赤湾%", "%南沙%",
];
...
async function listShipments(pool, cols, hasContainers, hasLines, q) {
  ...
  if (Number.isFinite(id) && cols.has("id")) {
    params.push(id);
    conds.push(`s.id = $${params.length}`);
  }
  const idExpr = cols.has("id") ? "s.id" : "NULL::text";
  const containerAgg = hasContainers && cols.has("id")
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺深圳/南沙口岸舱单申报通道、通道凭证、发送状态字段和回执字段；本页不对外发送。",
  };
}
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[shenzhen-nansha-manifest-send]", err);
