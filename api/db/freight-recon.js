...
import {
  ...
  writeException,
  writeExceptionAudit,
  writeSettlementAndAudit,
} from "./freight-recon-side-effects.js";

const AUTO_MATCH_TOLERANCE = 10;

// 副作用辅助已拆到独立文件，避免本活树文件继续超过 500 行。
  ...
  try {
    ...
    for (const link of links.rows) {
      ...
      if (!dryRun) {
        ...
        const sideEffects = await writeSettlementAndAudit(db, { link, paymentId, invoiceNo, amount: actual, reconChange, opId }, true);
        await recalcPlanFreightTotals(db, planId(plan));
        if (sideEffects?.side_effect_failures) {
          // settlement link failure is a system write fault, so return a visible marker instead of pretending full success.
          results.push({
            status: "side_effect_failed",
            link_id: link.id,
            invoice_no: invoiceNo,
            payment_id: paymentId,
            side_effect_failures: sideEffects.side_effect_failures,
          });
        }
      }
