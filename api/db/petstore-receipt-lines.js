  ...
  const sql = `
    WITH filtered AS (
      SELECT receipt_no, product_code, product_name, spec_text, qty_purchase,
       ...
       WHERE ($1::text IS NULL OR receipt_no = $1)
      ...
      SELECT receipt_no, product_code, product_name, spec_text, qty_purchase,
             ...
             produce_date, expire_date
        FROM filtered
       ORDER BY receipt_no DESC NULLS LAST, product_code ASC
       LIMIT $3 OFFSET $4
    )
    SELECT COALESCE(
             (
               SELECT jsonb_agg(to_jsonb(page_rows) ORDER BY receipt_no DESC NULLS LAST, product_code ASC)
                 FROM page_rows
             ),
             '[]'::jsonb
           ) AS rows,
