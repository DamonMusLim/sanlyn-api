...
import { scrubForCustomer, scrubCustomerNested, CUSTOMERS_CUSTOMER_SAFE } from "../lib/customer-safe-fields.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      // MODE_D_PREP_CUSTOMER_API_STRIPPER_001: customer/forwarder allowlist
      var safe = scrubForCustomer(result.rows, req.user?.role, CUSTOMERS_CUSTOMER_SAFE);
      safe = scrubCustomerNested(safe, req.user?.role);  // v2
    ...
    if (err.message.includes("does not exist") && req.method === "GET") {
      try {
        ...
        // MODE_D_PREP_CUSTOMER_API_STRIPPER_001: customer/forwarder allowlist on OSS fallback path too
        list = scrubForCustomer(list, req.user?.role, CUSTOMERS_CUSTOMER_SAFE);
        list = scrubCustomerNested(list, req.user?.role);  // v2
