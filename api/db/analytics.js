...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === "customer360") {
      ...
      const result = await pool.query(`
        SELECT o.customer,
          ...
          AVG(EXTRACT(EPOCH FROM (p.paid_date::timestamp - o.created_at::timestamp))/86400) AS avg_payment_days,
          COALESCE(SUM(CAST(p.received_amount AS NUMERIC)),0) AS total_received,
          SUM(CAST(o.total_amount AS NUMERIC)) - COALESCE(SUM(CAST(p.received_amount AS NUMERIC)),0) AS outstanding
        FROM orders o LEFT JOIN finance_payments p ON o.contract_no = p.contract_no
    ...
    if (type === "receivables") {
      const result = await pool.query(`
        SELECT o.contract_no, o.customer,
          CAST(o.total_amount AS NUMERIC) AS invoiced,
          COALESCE(CAST(p.received_amount AS NUMERIC),0) AS received,
          CAST(o.total_amount AS NUMERIC) - COALESCE(CAST(p.received_amount AS NUMERIC),0) AS outstanding,
          o.currency, o.created_at
        FROM orders o LEFT JOIN finance_payments p ON o.contract_no = p.contract_no
        WHERE CAST(o.total_amount AS NUMERIC) > 0
          AND (p.contract_no IS NULL OR CAST(o.total_amount AS NUMERIC) > COALESCE(CAST(p.received_amount AS NUMERIC),0))
