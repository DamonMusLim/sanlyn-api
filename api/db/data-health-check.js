...
export default async function handleDataHealthCheck(req, res) {
  ...
  try {
    const [r1, r2, r3, r4, r5, r6, r7, r8] = await Promise.all([
      ...
      pool.query(`WITH bad AS (SELECT COALESCE(order_no,id::text) okey, id FROM orders WHERE deleted_at IS NULL AND total_amount>0 AND declare_amount > total_amount*2) SELECT COUNT(*)::int AS cnt, ARRAY(SELECT okey FROM bad ORDER BY id LIMIT 5) AS samples FROM bad`),
    ...
    const checks = {
      ...
      // 🩸0812:体检只盯「写成0」,从来没盯过「大得离谱」——18 票申报额被写成 47 倍
      //   躺了几个月没人发现。⚖️ 检查缺失的方向,和检查错误的方向,是两件事。
      declare_amount_inflated:           { count: rInf.rows[0]?.cnt??0, samples: rInf.rows[0]?.samples??[], tier:'P0' },
