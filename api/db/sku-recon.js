...
async function restockIn(req, res, pool, r, scopeCodes) {
  if (!(r === "factory" || isInternal(r))) return json(res, 403, { success: false, error: "forbidden" });
  const sku = clean(req.body?.sku, 80);
  const delta = Number(req.body?.delta);
  if (!sku) return json(res, 400, { success: false, error: "sku required" });
  if (!Number.isFinite(delta) || delta === 0) return json(res, 400, { success: false, error: "delta invalid" });
  const client = await pool.connect();
  let product, before, after;
  try {
    await client.query("BEGIN");
    product = await scopedProduct(client, sku, r, scopeCodes, true);
    let fg = (await client.query(`SELECT * FROM finished_goods_inventory WHERE sku=$1 AND warehouse_id=1 FOR UPDATE`, [sku])).rows[0] || null;
    if (!fg) {
      await client.query(`INSERT INTO finished_goods_inventory(product_id, sku, unit, current_stock, safety_stock, factory_code, warehouse_id)
         VALUES($1,$2,$3,0,0,$4,1) ON CONFLICT (sku, warehouse_id) DO NOTHING`,
        [product.id, sku, product.unit || null, product.factory_code || null]);
      fg = (await client.query(`SELECT * FROM finished_goods_inventory WHERE sku=$1 AND warehouse_id=1 FOR UPDATE`, [sku])).rows[0];
    }
    before = Number(fg.current_stock || 0);
    after = before + delta;
    if (after < 0) throw new Error("delta invalid");
    await client.query(`UPDATE finished_goods_inventory SET current_stock=$1, last_move_at=NOW(), updated_at=NOW() WHERE id=$2`, [after, fg.id]);
    await client.query(`INSERT INTO inventory_logs(product_id, sku, type, quantity, unit, before_stock, after_stock, ref_type, ref_id, warehouse_id, factory_code, note, "operator")
       VALUES($1,$2,$3,$4,$5,$6,$7,'factory-restock',$8,1,$9,$10,$11)`,
      [product.id, sku, delta > 0 ? "in" : "out", delta, product.unit || null, before, after, "restock-" + Date.now(), product.factory_code || null, "工厂补充入库", actor(req)]);
    await client.query("COMMIT");
  } catch (e) {
    await client.query("ROLLBACK");
    throw e;
  ...
  try {
    await writeAudit(pool, req, { action: "sku-recon.restock-in", entity_type: "sku", entity_id: sku, before: { finished_stock: before }, after: { finished_stock: after, delta }, note: "工厂补充入库" });
  } catch (_) { /* 审计尽力而为 */ }
  return json(res, 200, { success: true, sku, before, after });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PATCH" && clean(req.query.action || req.body?.action, 40) === "restock-in") return await restockIn(req, res, pool, r, scopeCodes);
