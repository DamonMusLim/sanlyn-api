...
product_rows AS (
  SELECT so.order_no, 'product'::text AS category, oi.item_desc, so.bl_no,
...
export function buildSummary(rows) {
  const byCurrency = {};
  const orders = new Set(rows.map((r) => r.order_no).filter(Boolean));
...
// Final line count after this edit: 140.
