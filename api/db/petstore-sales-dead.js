...
async function listRows(req) {
  ...
  const minDays = nonNegativeInt(req.query?.min_days);
  const params = [storeCode, minDays, pageSize, offset];
  const sql = `
    WITH filtered AS (
      SELECT product_code, product_name, category_name, spec, cur_stock,
             days_since_last_sale, last_sale_at, restock_verdict, verdict_reason
        FROM public.petstore_sku_sales_dna
       WHERE (qty_30 = 0 OR qty_30 IS NULL)
         AND ($1::text IS NULL OR store_code = $1)
         AND ($2::int IS NULL OR days_since_last_sale >= $2::int)
    ), total_count AS (
      ...
      SELECT product_code, product_name, category_name, spec, cur_stock,
             days_since_last_sale, last_sale_at, restock_verdict, verdict_reason
        FROM filtered
       ORDER BY days_since_last_sale DESC NULLS LAST, product_code
       LIMIT $3 OFFSET $4
    )
    SELECT COALESCE(
