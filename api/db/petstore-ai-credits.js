...
async function listRows(req) {
  ...
  const sql = `
    ...
    page_rows AS (
      SELECT *, ROW_NUMBER() OVER (ORDER BY created_at DESC NULLS LAST, id DESC) AS __rn FROM filtered ORDER BY created_at DESC NULLS LAST, id DESC LIMIT $5 OFFSET $6
    )
    SELECT COALESCE(jsonb_agg(to_jsonb(page_rows) - '__rn' ORDER BY page_rows.__rn) FILTER (WHERE page_rows.id IS NOT NULL), '[]'::jsonb) AS rows,
           total_count.total
