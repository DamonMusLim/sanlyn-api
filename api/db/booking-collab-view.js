...
async function handleMasterView(req, res, pool) {
  ...
  // 货代(intermediary)默认洋宝宝(Damon 2026-07-18):raw未存时默认带出上海洋宝宝(居间货代),外单可手动选覆盖(存raw)。查库取id不硬编码。
  let interId = rawp2.intermediary_company_id || null;
  if (interId) {
    summary.intermediary_cn = await companyName(pool, interId);
  } else {
    const dr = await pool.query("SELECT id, COALESCE(name_cn,name_en) cn FROM companies WHERE code='OCEANBABY' OR (name_cn ILIKE '%上海洋宝宝%' AND is_sanlyn_entity=true) ORDER BY CASE WHEN code='OCEANBABY' THEN 0 ELSE 1 END LIMIT 1").catch(() => ({ rows: [] }));
    if (dr.rows[0]) { interId = dr.rows[0].id; summary.intermediary_cn = dr.rows[0].cn; summary.intermediary_default = true; }
  }
  summary.intermediary_company_id = interId;
