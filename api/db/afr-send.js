// /api/db/afr-send — Japan AFR declaration-channel read lens. Read-only.
...
const PLAN_FIELDS = [
  ...
  ["pol", "装港"], ["pod", "日本卸港"], ["etd", "ETD"], ["eta", "ETA"],
...
const LINE_FIELDS = [
  ...
  ["ctns", "箱数"], ["gw_kg", "毛重"], ["amount", "逐项货值"],
];
const SEND_FIELDS = [
  ["afr_send_status", "AFR发送状态"], ["afr_sent_at", "AFR发送时间"],
  ["afr_receipt_no", "AFR回执号"], ["afr_filer_code", "申报方代码"],
  ["afr_filing_type", "申报类型"],
];
const JP_HINTS = [
  "%JP%", "%JPN%", "%JAPAN%", "%TOKYO%", "%YOKOHAMA%", "%OSAKA%", "%KOBE%",
  "%NAGOYA%", "%HAKATA%", "%MOJI%", "%SHIMIZU%", "%KAWASAKI%", "%YOKKAICHI%",
];
...
async function listPlans(pool, planCols, hasCustoms, customsCols, hasLines, q) {
  ...
  if (search) {
    ...
    const cond = jpCond(planCols, params);
    if (cond) conds.push(cond);
  }
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺日本AFR/NACCS或第三方申报通道接口、通道凭证、申报方代码、发送状态字段和回执字段；本页不对外发送。",
  };
}
