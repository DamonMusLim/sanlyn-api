...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ds = clean(req.query?.dispense, 12);
    if (ds) { args.push(ds); where += ` AND t.dispense_status = $${args.length}`; }
    const total = (await pool.query(`SELECT count(*)::int n FROM clinic_prescriptions t LEFT JOIN pet_profiles p ON p.id = t.pet_id WHERE ${where}`, args)).rows[0].n;
    args.push(size, (page - 1) * size);
    const { rows } = await pool.query(
      `SELECT t.id, t.rx_no, t.medical_no, t.diagnosis, t.advice, t.vet_name, t.hospital,
     t.issued_at, t.dispense_status, t.ref_count, t.remark,
     t.owner_name, p.name AS pet_name, p.avatar_url, p.breed FROM clinic_prescriptions t LEFT JOIN pet_profiles p ON p.id = t.pet_id WHERE ${where} ORDER BY (t.dispense_status='待配药') DESC, t.issued_at DESC NULLS LAST
        LIMIT $${args.length - 1} OFFSET $${args.length}`, args);
    const sum = (await pool.query(
      `SELECT count(*) FILTER (WHERE dispense_status='待配药')::int pending
         FROM clinic_prescriptions WHERE store_code=$1`, [storeCode])).rows[0];
