...
async function listRows(req) {
  ...
  const params = [keyword, status, roleKey, storeCode, pageSize, offset];
  const sql = `
    WITH user_stores AS (
      SELECT account_id,
      ...
      SELECT a.id AS account_id, a.username, a.contact_name, a.contact_phone,
        ...
        FROM accounts a
        JOIN petstore_account_profile p ON p.account_id = a.id
