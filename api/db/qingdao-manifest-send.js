// /api/db/qingdao-manifest-send — Qingdao manifest send readiness lens. Read-only.
...
const SEND_FIELDS = [
  ["qingdao_manifest_status", "青岛舱单发送状态"],
  ["qingdao_manifest_sent_at", "青岛舱单发送时间"],
  ["qingdao_manifest_receipt_no", "青岛舱单回执号"],
];
const QINGDAO_HINTS = ["%QINGDAO%", "%TAO%", "%CNTAO%", "%青岛%"];
...
function qingdaoCond(cols, params) {
  const names = PORT_FIELDS.map(([name]) => name).filter((n) => cols.has(n));
  if (!names.length) return "";
  params.push(QINGDAO_HINTS);
...
async function listShipments(pool, cols, hasContainers, hasLines, q) {
  ...
  if (search) {
    ...
    const cond = qingdaoCond(cols, params);
    if (cond) conds.push(cond);
  }
  if (q.id && cols.has("id")) {
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺青岛口岸舱单申报通道、通道凭证、发送状态字段和回执字段；本页不对外发送。",
  };
}
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[qingdao-manifest-send]", err);
