// /api/db/ports.js — Ports master data CRUD (weight_limit_kg added)
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      const { code, name_en, name_cn, country_code, port_type, unlocode, note, weight_limit_kg, weight_limit_note } = req.body || {};
      if (!code || !name_en) return res.status(400).json({ error: "code and name_en required" });
      const r = await pool.query(
        `INSERT INTO ports(code,name_en,name_cn,country_code,port_type,unlocode,note,weight_limit_kg,weight_limit_note)
         VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
        [code.toUpperCase(), name_en, name_cn||null, (country_code||null)&&country_code.toUpperCase(), port_type||'sea', unlocode||null, note||null, weight_limit_kg||null, weight_limit_note||null]
      );
  ...
  if (req.method === "PATCH") {
    try {
      ...
      const allowed = ["name_en","name_cn","country_code","port_type","unlocode","note","weight_limit_kg","weight_limit_note"];
      const sets = [], vals = [];
      for (const k of allowed) {
        if (patch[k] !== undefined) {
          vals.push(k==="country_code"&&patch[k]?String(patch[k]).toUpperCase():patch[k]);
