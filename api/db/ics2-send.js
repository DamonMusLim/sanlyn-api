// /api/db/ics2-send — EU ICS2/ENS declaration-channel read lens. Read-only.
...
const PARTY_FIELDS = [
  ["shipper_name", "发货人"], ["shipper_address", "发货人地址"], ["shipper_eori", "发货人EORI"],
  ["consignee_name", "收货人"], ["consignee_address", "收货人地址"], ["consignee_eori", "收货人EORI"],
...
const SEND_FIELDS = [
  ["ics2_send_status", "ICS2发送状态"], ["ics2_sent_at", "ICS2发送时间"],
  ["ics2_lrn", "ICS2 LRN"], ["ics2_mrn", "ICS2 MRN"],
  ["ics2_filing_type", "申报类型"], ["ics2_filer_eori", "申报方EORI"],
];
const EU_HINTS = [
  "%EU%", "%EUROPE%", "%GERMANY%", "%HAMBURG%", "%BREMERHAVEN%", "%ROTTERDAM%",
  "%ANTWERP%", "%LE HAVRE%", "%FELIXSTOWE%", "%GDANSK%", "%GENOA%", "%VALENCIA%",
  "%BARCELONA%", "%PIRAEUS%", "%FRANCE%", "%ITALY%", "%SPAIN%", "%POLAND%",
  "%NETHERLANDS%", "%BELGIUM%", "%IRELAND%", "%SWEDEN%", "%DENMARK%", "%FINLAND%",
];
...
function rowOut(row, planCols, customsCols) {
  ...
  return {
    ...
    shipper_eori: row.shipper_eori, consignee_name: row.consignee_name, consignee_eori: row.consignee_eori,
    notify_name: row.notify_name, line_count: row.line_count == null ? null : Number(row.line_count),
...
async function listPlans(pool, planCols, hasCustoms, customsCols, hasLines, q) {
  ...
  if (search) {
    ...
    const cond = euCond(planCols, params);
    if (cond) conds.push(cond);
  }
...
function notConnected(rates) {
  return {
    ...
    fill_rates: rates,
    note: "缺ICS2/ENS申报通道接口、欧盟入境报文凭证、申报方EORI、发送状态字段和MRN/LRN回执字段；本页不对外发送。",
  };
}
