...
// W0-6: removed FALLBACK_RATE = 7.2 — see lib/finance/exchange-rate.js + memory
// feedback_never_invent_fields. When no rate is reachable, return 503 instead
// of a guessed 7.2 (wrong by ~1.5% on most days).
  ...
  if (req.method === "GET") {
    try {
      ...
      try {
        ...
        // W0-6: STOP — no cached row AND frankfurter unreachable. Don't fabricate.
        return res.status(503).json({
          success: false,
          pair,
          error: "exchange_rate_unavailable",
          refreshError: e.message,
          hint: "No cached rate; upstream frankfurter unreachable. Try again later or POST {action:'refresh'}.",
        });
      }
    } catch (e) {
